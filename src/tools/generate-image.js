...
export async function handleGenerateImage(args) {
  ...
  try {
    ...
    // The prompt submit button carries a material icon whose text is EXACTLY
    // "arrow_forward". Use :text-is (exact) not :has-text — the latter also
    // matches "arrow_forward_ios" (a chevron in the conversation) and plain
    // "Crea" matches "add_2Crea" (asset panel), both picked first by .first().
    const generateBtnLocator = page.locator('button:has(:text-is("arrow_forward"))').first();
